...
from bcos.utils import BcosUtilMixin, BcosSequential
from bcos.vit import SimpleViT
from bcos.experiments.utils import Experiment

__all__ = ["list_available", "BcosEncoderWrapper"]  # entrypoints filled by `register` decorator


_entrypoint_registry = {}


class BcosEncoderWrapper(BcosUtilMixin, torch.nn.Module):
    # wraps BCos models to forward only until after the feature extractor
    def __init__(
            self,
            model: torch.nn.Module,
    ):
        super().__init__()
        if isinstance(model, BcosSequential) and isinstance(model[0], SimpleViT):
            self.avgpool = lambda x: x.mean(dim=1)
            model = model[0]
        elif isinstance(model, SimpleViT):
            self.avgpool = lambda x: x.mean(dim=1)
        else:
        ...
        self.featurizer = model.get_feature_extractor()
        self.n_filters_out = model.num_features
