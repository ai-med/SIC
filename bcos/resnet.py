...
from bcos.utils import BcosUtilMixin, update_default_layers
from bcos.bcosconv2d import BcosConv2d
from bcos.bcoslinear import BcosLinear
from bcos.logitlayer import LogitLayer
from bcos import norms
...
class BcosResNet(BcosUtilMixin, nn.Module):
    def __init__(
        ...
        **kwargs: Any,  # ignore rest
        ...
        self._conv_layer = update_default_layers(conv_layer, kwargs)
        if kwargs:
...
def _resnet(
    ...
    **kwargs: Any,
    ...
    if 'in_chans' in kwargs.keys():
        kwargs['in_chans'] = 2 * kwargs['in_chans']
