...
def update_default_layers(layers: Dict[str, Any], kwargs: Dict[str, Any]):
    if "bcos_args" in kwargs.keys():
        bcos_args = kwargs.pop("bcos_args")
        print("updating bcos args: ", bcos_args)
        if isinstance(layers, list):
            return [partial(layer, **bcos_args) for layer in layers]
        else:
            return partial(layers, **bcos_args)
    else:
        return layers
...
class BcosUtilMixin:
    ...
    def explain_featurizer(
        ...
        **grad2img_kwargs,
        ...
        """
        Generates an explanation for the given input tensor and the latent feature vector
        after global average pooling of the featurizer.
        ...
        in_tensor : Tensor
            The input tensor to explain. Must be 4-dimensional and have batch size of 1.
        idx : int
            The index of the latent feature vector to explain. If None, the prediction is explained.
        grad2img_kwargs : Any
            Additional keyword arguments passed to `gradient_to_image` method
            for generating the explanation.
        ...
        Dict[str, Any]
            A dictionary containing the explanation and additional information.
            Namely, the following keys are present:
            - "feature_index": The class (index) of the explained output.
            - "dynamic_linear_weights": The dynamic linear weights of the model (`in_tensor.grad`).
            - "contribution_map": The contribution map of the model prediction.
            - "explanation": The explanation of the model prediction.
        """
        ...
        with torch.enable_grad(), self.explanation_mode():
            # fwd + prediction
            out = self.featurizer(in_tensor)  # noqa
    ...
    def gradient_to_image(
        ...
        clip_negative: bool
            Whether to only visualize negative attributing features
        ...
        """
        return gradient_to_image(image, linear_mapping, smooth, alpha_percentile)  #, clip_negative)
...
def gradient_to_image(image, linear_mapping, smooth=15, alpha_percentile=99.5):
    ...
    np.ndarray
        ...
        Shape: [H, W, C] (C=4 ie RGBA)
    """
    assert image.size(0) <= 6, "Expected <= 6 channels in input image."
    channel_select = 3 if (image.size(0) % 3 == 0) else 1
    ...
    rgb_grad = rgb_grad[:channel_select] / (rgb_grad[:channel_select] + rgb_grad[channel_select:] + 1e-12)  # [3, H, W]
...
def plot_contribution_map(
    ...
    if percentile is not None:
        ...
        # assert cutoff == np.abs(contribution_map).max()
