...
from .utils import DetachableModule
...
class SimpleViT(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if not linear_layer is nn.Linear:  # if it's nn.Linear, this is not a B-cos model and doesn't require 1-RGB
            channels = 2 * in_chans if in_chans % 2 != 0 else in_chans  # 1 and 3 channel inputs need to be scaled
        else:
            channels = in_chans
    ...
    def get_feature_extractor(self) -> nn.Module:
        """Returns the feature extractor part of the model.
            Note that this is only called by goat models.
        """
